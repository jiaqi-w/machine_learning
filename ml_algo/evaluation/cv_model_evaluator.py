...
class CV_Model_Evaluator():
    ...
    def get_evaluation_dict(self, evaluation_fname=None):
        cv_metric_dict = {}
        cv_metric_dict["cv_accuracy"] = np.average(self.accuracy_list)
        cv_metric_dict["cv_roc_auc"] = np.average(self.roc_auc_list)
        cv_metric_dict["cv_macro_prec"] = np.average(self.macro_precision_list)
        cv_metric_dict["cv_macro_recall"] = np.average(self.macro_recall_list)
        cv_metric_dict["cv_macro_f1"] = np.average(self.macro_f1_list)
        cv_metric_dict["cv_micro_prec"] = np.average(self.micro_precision_list)
        cv_metric_dict["cv_micro_recall"] = np.average(self.micro_recall_list)
        cv_metric_dict["cv_micro_f1"] = np.average(self.micro_f1_list)
        cv_metric_dict["cv_weighted_prec"] = np.average(self.weighted_precision_list)
        cv_metric_dict["cv_weighted_recall"] = np.average(self.weighted_recall_list)
        cv_metric_dict["cv_weighted_f1"] = np.average(self.weighted_f1_list)
